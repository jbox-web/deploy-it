...
module DockerServices
  class AppCompiler
    ...
    def initialize(application, archive_path, logger)
      @application   = application
      @archive_path  = archive_path
      @logger        = logger
      @image_type    = application.image_type
      @image_name    = application.image_name
    end


    ## Called by build.rb. This build the docker image
    def run!
      cleanup
      receive(docker_options_for(:receive))
    ...
    def receive(docker_options = {})
      ...
      new_image_from_file(image_type, image_name, archive_path, docker_options)
    ...
    private
      ...
      def execute_plugins(step, docker_options = {})
        Plugins.execute(step, application, docker_connection, logger, docker_options)
      end
